				...
				<ul id="auth-tools">
					<li id="auth-yourfavourites" class="no-js">
						...
						<ul id="auth-dash-favourites" class="auth-dash">
							...
							$querystr = "SELECT wp_posts.*, m1.meta_value as _thumbnail_id FROM wp_posts LEFT JOIN wp_usermeta as m0 on REPLACE(m0.meta_key, 'likepost', '')=wp_posts.ID left join wp_postmeta as m1 on m1.post_id=wp_posts.ID and m1.meta_key='_thumbnail_id' WHERE post_status='publish' AND m0.meta_value > 0 AND m0.user_id = $current_user->ID AND m0.meta_key LIKE 'likepost%' AND m1.meta_value >= 1 ORDER BY m0.meta_value DESC LIMIT 5;";
